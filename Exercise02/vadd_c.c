...
#define CL_USE_DEPRECATED_OPENCL_1_2_APIS //If you need your code to run on devices that don't yet support OpenCL 2.0, 
                                          //you can continue using the deprecated clCreateCommandQueue function 
                                          //by using the preprocessor macros that the OpenCL headers provide
#ifdef __APPLE__
...
{
    char *KernelSource = getKernelSource("vector_add.cl");
...
char * getKernelSource(char *filename)
{
    ...
    size_t result = fread(source, sizeof(char), len, file);
    if (result != ftell(file)) {printf("Reading Fail!\n");}
    printf("%s\n", source);
    fclose(file);
    return source;
}
